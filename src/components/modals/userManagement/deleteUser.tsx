...
import { IMockUser, IUser, ModifyUsersRole } from "@/models/user.model";
...
export default function DeleteUserModal({
  ...
  const deleteUser = async (id: number) => {
    ...
    try {
      ...
      if (res.status === "OK") {
        toast({
          status: "success",
          description: `user ${users[0].name} has been deleted`,
  ...
  const deleteMultipleUsers = async () => {
    ...
    try {
      const res = await OnboardingServices.deleteMultipleUsers(data);
      if (res.status === "OK") {
        toast({
          status: "success",
      ...
      <ModalContent className="p-2 ">
        ...
        <ModalBody className="w-full  rounded-lg bg-white">
          <div className="flex flex-col gap-5">
            ...
            <p className="text-[#474A57]">
              You are about to remove the user{isSingleUSer(users) ? "" : "s"}
              {isSingleUSer(users) ? (
                ...
                users.map((item, index) => (
                  <span className="font-semibold text-dark-grey" key={index}>
                    {" "}
              ...
              . If you continue,{" "}
              {isSingleUSer(users) ? "the user" : "these users"} will no longer
              be able to log on to this application.
            </p>
        ...
        <ModalFooter>
          <div className="w-full flex justify-end gap-3">
            ...
            >
              {loading ? <Spinner size={"sm"} /> : "Yes, Remove"}
            </button>
          </div>
        </ModalFooter>
      </ModalContent>
